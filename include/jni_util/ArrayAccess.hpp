...
namespace jni_util {
    ...
    struct ArrayFunctions<jboolean> {
        ...
        static Elem *GetArrayElements(JNIEnv *env, jArray arrayj, jboolean *isCopy) {
            return env->GetBooleanArrayElements(arrayj, isCopy);
        }

        static void ReleaseArrayElements(JNIEnv *env, jArray arrayj, Elem *elems, jint mode) {
            env->ReleaseBooleanArrayElements(arrayj, elems, mode);
        }

        static void GetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) {
            env->GetBooleanArrayRegion(arrayj, start, len, buf);
        }

        static void SetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) {
            env->SetBooleanArrayRegion(arrayj, start, len, buf);
        }
    };
    ...
    struct ArrayFunctions<jbyte> {
        ...
        static Elem *GetArrayElements(JNIEnv *env, jArray arrayj, jboolean *isCopy) {
            return env->GetByteArrayElements(arrayj, isCopy);
        }

        static void ReleaseArrayElements(JNIEnv *env, jArray arrayj, Elem *elems, jint mode) {
            env->ReleaseByteArrayElements(arrayj, elems, mode);
        }

        static void GetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) { env->GetByteArrayRegion(arrayj, start, len, buf); }

        static void SetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) { env->SetByteArrayRegion(arrayj, start, len, buf); }
    };
    ...
    struct ArrayFunctions<jchar> {
        ...
        static Elem *GetArrayElements(JNIEnv *env, jArray arrayj, jboolean *isCopy) {
            return env->GetCharArrayElements(arrayj, isCopy);
        }

        static void ReleaseArrayElements(JNIEnv *env, jArray arrayj, Elem *elems, jint mode) {
            env->ReleaseCharArrayElements(arrayj, elems, mode);
        }

        static void GetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) { env->GetCharArrayRegion(arrayj, start, len, buf); }

        static void SetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) { env->SetCharArrayRegion(arrayj, start, len, buf); }
    };
    ...
    struct ArrayFunctions<jshort> {
        ...
        static Elem *GetArrayElements(JNIEnv *env, jArray arrayj, jboolean *isCopy) {
            return env->GetShortArrayElements(arrayj, isCopy);
        }

        static void ReleaseArrayElements(JNIEnv *env, jArray arrayj, Elem *elems, jint mode) {
            env->ReleaseShortArrayElements(arrayj, elems, mode);
        }

        static void GetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) { env->GetShortArrayRegion(arrayj, start, len, buf); }

        static void SetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) { env->SetShortArrayRegion(arrayj, start, len, buf); }
    };
    ...
    struct ArrayFunctions<jint> {
        ...
        static Elem *GetArrayElements(JNIEnv *env, jArray arrayj, jboolean *isCopy) {
            return env->GetIntArrayElements(arrayj, isCopy);
        }

        static void ReleaseArrayElements(JNIEnv *env, jArray arrayj, Elem *elems, jint mode) {
            env->ReleaseIntArrayElements(arrayj, elems, mode);
        }

        static void GetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) { env->GetIntArrayRegion(arrayj, start, len, buf); }

        static void SetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) { env->SetIntArrayRegion(arrayj, start, len, buf); }
    };
    ...
    struct ArrayFunctions<jlong> {
        ...
        static Elem *GetArrayElements(JNIEnv *env, jArray arrayj, jboolean *isCopy) {
            return env->GetLongArrayElements(arrayj, isCopy);
        }

        static void ReleaseArrayElements(JNIEnv *env, jArray arrayj, Elem *elems, jint mode) {
            env->ReleaseLongArrayElements(arrayj, elems, mode);
        }

        static void GetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) { env->GetLongArrayRegion(arrayj, start, len, buf); }

        static void SetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) { env->SetLongArrayRegion(arrayj, start, len, buf); }
    };
    ...
    struct ArrayFunctions<jfloat> {
        ...
        static Elem *GetArrayElements(JNIEnv *env, jArray arrayj, jboolean *isCopy) {
            return env->GetFloatArrayElements(arrayj, isCopy);
        }

        static void ReleaseArrayElements(JNIEnv *env, jArray arrayj, Elem *elems, jint mode) {
            env->ReleaseFloatArrayElements(arrayj, elems, mode);
        }

        static void GetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) { env->GetFloatArrayRegion(arrayj, start, len, buf); }

        static void SetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) { env->SetFloatArrayRegion(arrayj, start, len, buf); }
    };
    ...
    struct ArrayFunctions<jdouble> {
        ...
        static Elem *GetArrayElements(JNIEnv *env, jArray arrayj, jboolean *isCopy) {
            return env->GetDoubleArrayElements(arrayj, isCopy);
        }

        static void ReleaseArrayElements(JNIEnv *env, jArray arrayj, Elem *elems, jint mode) {
            env->ReleaseDoubleArrayElements(arrayj, elems, mode);
        }

        static void GetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) {
            env->GetDoubleArrayRegion(arrayj, start, len, buf);
        }

        static void SetArrayRegion(JNIEnv *env, jArray arrayj, jsize start, jsize, jsize len,
                                   Elem *buf) {
            env->SetDoubleArrayRegion(arrayj, start, len, buf);
        }
    };
    ...
    public:
        ...
        CopiedArrayAccess(JNIEnv *env, jArray arrayj, jint mode = 0)
                : env_(env), arrayj_(arrayj), mode_(mode) {
            assert(env_ != nullptr);
